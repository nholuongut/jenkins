...
//
//  [% URL  # https://github.com/nholuongut/jenkins %]
//
//  [% LICENSE %]
//
//  [% MESSAGE %]
...
//@Library(value='nholuongut@master', changelog=false) _
...
//  agent {
    ...
    //
    //  https://github.com/nholuongut/Dockerfiles/tree/master/jenkins-agent-docker
    //  https://github.com/nholuongut/Kubernetes-templates/blob/master/jenkins-agent.cloud-pod-DooD.yaml
    //
...
//      //
//      //    https://github.com/nholuongut/Kubernetes-templates/blob/master/jenkins-agent-pod.yaml
//      //
...
//  }
  ...
  environment {
    ...
    GITOPS_REPO = 'git@github.com/nholuongut/app-k8s'
    ...
    GHCR_REGISTRY = 'ghcr.io/nholuongut'
    //DOCKER_IMAGE = "$GHCR_REGISTRY/$APP"
    // DockerHub
    //DOCKER_IMAGE = "nholuongut/$APP"
    ...
    // if run on K8s through an ingress (see https://github.com/nholuongut/Kubernetes-templates/)
  ...
  stages {
    ...
    stage ('Checkout') {
      steps {
        ...
        //container('git') {
        //  git credentialsId: 'GitHub', url: 'https://github.com/nholuongut/jenkins.git', branch: 'master'
        //}
      }
    }
        ...
        lock(resource: 'Git Merge Staging to Dev', inversePrecedence: true) {
          ...
          timeout(time: 5, unit: 'MINUTES') {
            ...
            sshagent (credentials: ['jenkins-ssh-key-for-github']) {
              retry(2) {
                sh 'path/to/git_merge_branch.sh staging dev'  // script in https://github.com/nholuongut/devops-bash-tools - or see vars/gitMerge*.groovy for native shared libary
              }
            }
          }
        }
    ...
    stage('Build') {
      ...
      steps {
        ...
        timeout(time: 60, unit: 'MINUTES') {
          sh 'make'
          // or
          sh './gcp_ci_build.sh'  // script in https://github.com/nholuongut/devops-bash-tools
  ...
  // Deploys are intentionally below Approval
      ...
      steps {
        lock(resource: "Deploy - App: $APP, Environment: $ENVIRONMENT", inversePrecedence: true) {
          ...
          timeout(time: 15, unit: 'MINUTES') {
            ...
            sh './gcp_ci_deploy_k8s.sh'  // https://github.com/nholuongut/devops-bash-tools
            // OR
    ...
    stage('Deploy Production') {
      ...
      steps {
        ...
        sh 'path/to/gcp_ci_deploy_k8s.sh'  // https://github.com/nholuongut/devops-bash-tools
        // OR
        argoDeploy()  // func in vars/ shared library
      }
    }
